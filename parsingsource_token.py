...
types = [jlt.FormalParameter, jlt.BasicType, jlt.PackageDeclaration, jlt.InterfaceDeclaration, jlt.CatchClauseParameter,
         jlt.ClassDeclaration, jlt.MemberReference, jlt.SuperMemberReference, jlt.ConstructorDeclaration,
         jlt.ReferenceType,
...
types_CPDP = [jlt.FormalParameter, jlt.BasicType, jlt.PackageDeclaration, jlt.InterfaceDeclaration,
              jlt.CatchClauseParameter,
              jlt.ClassDeclaration, jlt.MethodInvocation, jlt.SuperMethodInvocation, jlt.MemberReference,
              ...
              jlt.ContinueStatement, jlt.ReturnStatement, jlt.ThrowStatement, jlt.SynchronizedStatement,
              jlt.TryStatement,
...
def append_suffix(df):
    for i in range(len(df)):
        try:
            ...
            try:
                df.loc[i, 'Class'] = df.loc[i, 'Class'] + ".java"
            except:
                df.loc[i, 'name'] = df.loc[i, 'name'] + ".java"
    return df
...
def extract_handcraft_instances(path):
    ...
    try:
        ...
        try:
            handcraft_instances = np.array(handcraft_instances['Class'])
        except:
            handcraft_instances = np.array(handcraft_instances['name'])
...
def extract_data(path_handcraft_file, dict_encoding_vector):
    def extract_label(df, file_name):
        try:
            row = df[df.file_name == file_name]['bugs']
        except:
            try:
                row = df[df.name1 == file_name]['bugs']
            except:
                row = df[df.name == file_name]['bugs']
    ...
    def extract_feature(df, file_name):
        try:
            ...
            try:
                row = df[df.name1 == file_name][features]
            except:
                row = df[df.name == file_name][features]
